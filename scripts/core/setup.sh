...
configure_passwordless_sudo() {
    ...
    if [[ "${DRY_RUN:-false}" == "true" ]]; then
        log_info "[DRY-RUN] Would configure passwordless sudo for current user and service user: $SERVICE_USER"
        return 0
    fi
    ...
    # Define the specific sudo commands that both users need
    local sudo_commands="/usr/bin/systemctl, /usr/bin/apt-get, /usr/bin/ufw, /bin/mkdir, /bin/chown, /bin/chmod, /usr/bin/curl, /usr/bin/mv, /usr/sbin/useradd, /usr/sbin/usermod, /usr/bin/docker, /usr/bin/certbot, /usr/sbin/nginx, /usr/bin/fail2ban-client"
    
    log_info "Detected current user: $current_user"
    log_info "Configuring sudoers for current user and service user: $SERVICE_USER"
    
    # Create sudoers configuration for both current user and service user
    cat > /tmp/jarvis-stack-sudoers << EOF
# JarvisJR Stack sudo configuration
# Generated automatically - do not edit manually

# Current user: $current_user (needed for service user creation and system setup)
$current_user ALL=(ALL) NOPASSWD: $sudo_commands

# Service user: $SERVICE_USER (needed for container operations and service management)
...
_show_manual_sudo_instructions() {
    ...
    log_info "======================================"
    log_info ""
    log_info "Automated sudo configuration failed. You have multiple remediation options:"
    log_info ""
    log_info "OPTION 1: Manual sudo configuration (RECOMMENDED)"
    log_info "  Step-by-step commands to run:"
    ...
    log_info "  4. sudo visudo -c  # Verify syntax (important!)"
    log_info "  5. ./jstack.sh     # Re-run installation"
    log_info ""
    log_info "OPTION 2: Use the dedicated sudo configuration command (AUTOMATED)"
    log_info "  ./jstack.sh --configure-sudo"
    log_info "  This will attempt the same configuration automatically"
    log_info ""
    log_info "OPTION 3: Force installation with password prompts (NOT RECOMMENDED)"
    log_info "  ./jstack.sh --force-install"
    log_info "  WARNING: This requires multiple password entries during installation"
    log_info "  and is not suitable for automated deployments"
    log_info ""
    log_info "OPTION 4: Alternative user-based approach"
    log_info "  If current user lacks sufficient privileges:"
    log_info "  a) Switch to a user with sudo access, or"
    log_info "  b) Ask system administrator to run: sudo usermod -aG sudo $USER"
    log_info "  c) Logout and login again to refresh group membership"
    log_info ""
    log_info "SECURITY INFORMATION:"
    log_info "====================="
    log_info "This configuration grants passwordless sudo for specific commands only:"
    log_info "- systemctl (service management)"
    log_info "- apt-get (package installation)"
    log_info "- ufw (firewall configuration)"
    log_info "- docker (container management)"
    log_info "- certbot (SSL certificate management)"
    log_info "- File operations (mkdir, chown, chmod, mv)"
    log_info ""
    log_info "Review the generated file at /tmp/jarvis-stack-sudoers before installation"
    log_info "if you have security concerns. This is significantly more secure than"
    log_info "granting full passwordless sudo access."
    log_info ""
    log_info "TROUBLESHOOTING:"
    log_info "================"
    log_info "If you encounter issues:"
    log_info "- Ensure you have write access to /etc/sudoers.d/"
    log_info "- Verify the syntax with 'sudo visudo -c' before proceeding"
    log_info "- Check system logs if installation continues to fail"
    log_info "- Contact support with error details if needed"
    log_info ""
}
...
# Create Docker Compose compatibility wrapper for Docker Desktop
create_docker_compose_wrapper() {
    log_info "Creating docker-compose compatibility wrapper"
    
    # Create wrapper script that forwards all commands to 'docker compose'
    cat > /tmp/docker-compose << 'EOF'
#!/bin/bash
# Docker Compose Compatibility Wrapper
# Automatically generated by JarvisJR Stack
# Forwards all commands to 'docker compose' (Docker Desktop plugin)
exec docker compose "$@"
EOF
    ...
    log_success "Docker Compose wrapper created successfully"
}
...
install_docker_compose_standalone() {
    ...
    execute_cmd "sudo chown root:root /usr/local/bin/docker-compose" "Set Docker Compose ownership"
    
    log_success "Standalone Docker Compose installed successfully"
}
